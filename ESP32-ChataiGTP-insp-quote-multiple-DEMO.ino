...
void printQuote() {
  ...
  String requestData = "{\"prompt\": \"Give me an inspirational quote\", \"max_tokens\": 40}";
  int httpResponseCode = http.POST(requestData);

if (httpResponseCode > 0) {
    // Parse the response as JSON
    const size_t bufferSize = JSON_OBJECT_SIZE(1) + JSON_OBJECT_SIZE(2) + JSON_OBJECT_SIZE(3) + 1000;
...
void loop() {
  ...
  http.addHeader("Content-Type", "application/json");
  http.addHeader("Authorization", String("Bearer ") + apiKey);
  String requestData = "{\"prompt\": \"Tell me an inspiring quote\", \"max_tokens\": 30}";
  int httpResponseCode = http.POST(requestData);

  if (httpResponseCode > 0) {
    // Parse the response as JSON
    const size_t bufferSize = JSON_OBJECT_SIZE(1) + JSON_OBJECT_SIZE(2) + JSON_OBJECT_SIZE(3) + 800;
    DynamicJsonDocument jsonDocument(bufferSize);
    DeserializationError error = deserializeJson(jsonDocument, http.getStream());
